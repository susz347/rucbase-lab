...
class IndexNSG : public Index {
 ...
 protected:
    ...
    void sync_prune(unsigned q, std::vector<Neighbor>& pool,
                    const Parameters &parameter, std::vector<unsigned char>& flags,
                    SimpleNeighbor* cut_graph_);
    void Link(const Parameters &parameters, SimpleNeighbor* cut_graph_);
    void Load_nn_graph(const char *filename);
    void tree_grow(const Parameters &parameter);
    void DFS(std::vector<unsigned char> &flag, unsigned root, unsigned &cnt);
    void findroot(std::vector<unsigned char> &flag, unsigned &root,
                  const Parameters &parameter);
