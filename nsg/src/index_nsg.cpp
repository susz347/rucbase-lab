...
void IndexNSG::get_neighbors(const float *query, const Parameters &parameter,
  ...
  std::vector<unsigned char> flags(nd_, 0);
  L = 0;
...
#pragma omp for schedule(dynamic, 100)
    for (int n = 0; n < (int)nd_; ++n) {
      pool.clear();
      tmp.clear();
      flags.assign(nd_, 0);
      get_neighbors(data_ + dimension_ * (size_t)n, parameters, flags, tmp, pool);
      sync_prune((unsigned)n, pool, parameters, flags, cut_graph_);
    }
  }

#pragma omp parallel for schedule(dynamic, 100)
  for (int n = 0; n < (int)nd_; ++n) {
    InterInsert((unsigned)n, range, locks, cut_graph_);
  }
}
...
void IndexNSG::DFS(std::vector<unsigned char> &flag, unsigned root, unsigned &cnt) {
  unsigned tmp = root;
...
void IndexNSG::findroot(std::vector<unsigned char> &flag, unsigned &root,
                        const Parameters &parameter) {
  unsigned id = nd_;
  for (unsigned i = 0; i < nd_; i++) {
    if (flag[i] == 0) {
